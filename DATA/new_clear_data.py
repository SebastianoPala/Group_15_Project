...
def manage_neo4j_indexes(session, action="CREATE"):
    if action == "CREATE":
        print("   -> Creating indexes on User.user_id and Game.game_id...")
        session.run("CREATE INDEX user_id_idx IF NOT EXISTS FOR (u:User) ON (u.user_id)")
        session.run("CREATE INDEX game_id_idx IF NOT EXISTS FOR (g:Game) ON (g.game_id)")
...
def main():
    ...
    neo4j_games = [{"game_id": g["_id"]["$oid"], "name": g.get("name",""), "achievements": g.get("achievements",0), "image": g.get("image","")} for g in games_list]
    ...
    try:
        ...
        with driver.session() as session:
            ...
            session.run("UNWIND $games AS game CREATE (g:Game {game_id: game.game_id, name: game.name, achievements: game.achievements, image: game.image})", games=neo4j_games)
            ...
            for i in range(0, len(played_relationships), 10000):
                session.run("UNWIND $rels AS rel MATCH (u:User {user_id: rel.user_id}) MATCH (g:Game {game_id: rel.game_id}) CREATE (u)-[:PLAYED {hoursPlayed: rel.hoursPlayed, achievements: rel.achievements}]->(g)", rels=played_relationships[i:i+10000])
