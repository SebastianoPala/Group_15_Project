...
public class AdminService {
    ...
    public void addGame(@Valid EditGameDTO newGame) {
        if(gameRepository.existsByName(newGame.getName()))
                throw new RuntimeException("Game "+ newGame.getName() +" already exists");
    ...
    public void editGame(String gameId, EditGameDTO editGame) {
        ...
        Game game = gameRepository.findById(gameId).orElseThrow(() -> new RuntimeException("The Game with id:\"" + gameId + "\" does not exist"));
        // is there a better way??
        if (editGame.getName() != null) {
            if(gameRepository.existsByName(editGame.getName()))
                throw new RuntimeException("Game "+ editGame.getName() +" already exists");
